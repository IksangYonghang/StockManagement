...
import { IBalanceSheet } from "../../types/global.typing";
...
const BalanceSheet = () => {
  ...
  const handleShowReport = async () => {
    ...
    try {
      const response = await httpModule.get(
        `FinalReport/GetBalanceSheet?isDuration=${isDuration}&fromDate=${convertToEnglishDigits(
          selectedFromDate
      ...
      setError(
        error.response?.data ||
          "An error occured while fetching balance sheet data"
      );
      setError(null);
    }
  };
  ...
  return (
    <>
      <h1 style={{ marginBottom: "2rem", fontSize: "22px" }}>Balance Sheet</h1>
